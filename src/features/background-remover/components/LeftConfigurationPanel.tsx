...
interface Props {
  ...
  runRemoval: () => void;
  executionStats: { durationMs: number; width: number; height: number; size: number } | null;
  stageText: string;
  bytesInfo: { total: number; current: number } | null;
...
export default function LeftConfigurationPanel(props: Props) {
  const {
    executionStats, isBusy, onClear, progressPercent, runRemoval, bytesInfo,
    stageText, formatBytes, model, setModel, setDevice, setOutputMode,
    backdrop, device, customBgColor, handleDownloadComposite, outputMode,
    setBackdrop, setCustomBgColor, handleDownload,
  } = props;
  ...
  return (
    <div className="lg:col-span-1 space-y-5 border border-border bg-surface rounded-lg p-5">
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-text-main">Inference Hardware</label>
      ...
      <DownloadActionButtons
        ...
        isBusy={isBusy}
        outputMode={outputMode}
        processedBlob={Boolean(executionStats)}
      />
